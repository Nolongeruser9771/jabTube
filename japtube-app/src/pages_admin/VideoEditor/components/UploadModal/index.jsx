...
export default function UploadModal({ filmId, handleClose, open, videoId, setVideoBlobUrl }) {
    //Modal
    const style = {
        position: 'absolute',
        top: '50%',
        ...
        if (selectedFile.type === 'video/mp4') {
            ...
            for (let chunkIdex = 0; chunkIdex < totalChunks; chunkIdex++) {
                ...
                const upload = await fetch(
                    API_URL +
                        `/api/v1/admin/videos/upload/chunk?filmId=${filmId}&videoId=${videoId}&chunkIndex=${chunkIdex}&totalChunks=${totalChunks}`,
                    {
                        method: 'POST',
                        headers: authHeader(),
                        body: formData,
