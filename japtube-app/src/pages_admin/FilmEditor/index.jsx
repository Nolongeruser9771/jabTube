...
export default function FilmEditor({ filmData }) {
    ...
    useEffect(() => {
        const getThumbnail = async () => {
            if (filmData.thumbnail) {
                try {
                    const thumbnailFetch = await fetch(API_URL + filmData.thumbnail)
    ...
    useEffect(() => {
        setValue('title', filmData.title)
        setValue('level', filmData.level)
        setValue('totalEpisode', filmData.totalEpisode)
        setValue('description', filmData.description)
        setValue('categoryIdList', filmData.categoryPublics)
        setValue('status', filmData.status)
        setValue('thumbnail', filmData.thumbnail)
    }, [filmData])
    ...
    const onSubmit = async (data) => {
        try {
            ...
            console.log(request)
            const result = await fetch(API_URL + `/api/v1/admin/films?filmId=${filmData.id}`, {
                method: 'PUT',
                headers: {
